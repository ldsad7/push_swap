...
/*   Updated: 2019/02/28 03:26:57 by tsimonis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		right_space(char *tmp, int flags[11], int num, int fd)
{
	int		ret;
	int		i;
	int		fl;
...
int				print_int(va_list ap, char ls[3], int flags[11], int fd)
{
	long long int	c;
	int				ret;
	int				num;
	...
	if (flags[6])
		...
		ret = right_space(tmp, flags, num, fd);
	ft_strdel(&tmp);
	return (ret);
}
